...
for ($n = 0; $n -lt $csvData.count-1; $n++){
    $earliestRecords = ($csvData | Sort-Object DateTime | Select-Object -First (1+$n)).DateTime
    ...
    #$trainingData = @($trainingData)  # Ensure $data is treated as an array DO WE NEED THIS??
    $trainingData | ForEach-Object { $_.price = [double]$_.price } 

    $bolleringBandsParameters = @()
    # Perform grid search returning the Bolliner Bands parameters for each combination of Rolling Window and Standard Deviation Multiplier
    ...
    # Loop through all the parameters we just discovered and see which one performs best on our training data    
